    ...
    <div className="max-w-5xl mx-auto px-6 py-16">
      ...
      <div className="space-y-12">
        {page.sections.map((section, index) => {
          // Check if this is a section-header only (like "Collaborations")
          const isSectionHeader = !section.content && !section.images && !section.metrics && !section.bullets;

          return (
            <section key={index} className={isSectionHeader ? 'pt-8' : 'border-t border-gray-100 pt-8'}>
              <h2 className={`font-bold text-gray-900 ${isSectionHeader ? 'text-3xl text-center uppercase tracking-wide' : 'text-2xl'} mb-4`}>
                {section.heading}
              </h2>
                ...
                <div className={`mt-6 grid gap-4 ${
                  ...
                  {section.images.map((img, i) => (
                    <ImageLightbox
                      ...
                      src={img.src}
                      alt={img.alt}
                      className="relative aspect-[4/3] rounded-lg overflow-hidden border border-gray-200 bg-gray-50"
                    />
                  ))}
                </div>
              )}
